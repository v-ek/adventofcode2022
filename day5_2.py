...
for stack in zip(*initial_state):
    # Make a list of each stack of, and store in a state dict with keys
    state[stack[0].strip()] = (
        [
...
for line in instructions:
    ...
    state[stack_to].extend(state[stack_from][-elements_to_move:])
    state[stack_from] = state[stack_from][:-elements_to_move]
